...
public class VoucherDao extends Dao {
	...
	public void add(Voucher voucher) throws SQLException {
		...
		boolean result = execute(sqlString,
				voucher.getProductionName(),
				voucher.getProductionCompany(),
				voucher.getRate(),
				voucher.getStartDate(),
				voucher.getEndDate(),
				Time.valueOf(voucher.getStartTime()),
				Time.valueOf(voucher.getEndTime()),
				voucher.getIsPaid());
	}
	...
	public void update(Voucher voucher) throws SQLException {
		...
		boolean result = execute(sqlString, voucher.getProductionName(), voucher.getProductionCompany(), voucher.getRate(),
				voucher.getStartDate(), voucher.getEndDate(), voucher.getStartTime(), voucher.getEndTime());
	}
	...
	public Voucher getVoucher(LocalDate date) throws Exception {
		String sqlString = String.format("SELECT * FROM %s WHERE %s = %s", TABLE_NAME, 
				VoucherFields.START_DATE.getFieldTitle(), java.sql.Date.valueOf(date));
		...
		try {
			...
			return new Voucher(resultSet.getString(VoucherFields.PRODUCTION_NAME.getFieldTitle()), resultSet.getString(VoucherFields.PRODUCTION_COMPANY.getFieldTitle()),
